...
const endLive = async (
  ...
  // Auto-generate recording path based on IVS structure
  if (!payload?.recordingUrl && stream.startedAt) {
    ...
    // Generate recording path pattern (IVS uses this structure)
    // Actual folder name will be generated by IVS, this is an approximation
    updateData.recordingUrl = `/ivs/v1/${accountId}/${channelId}/${year}/${month}/${day}/${hour}/${minute}`;
  } else if (payload?.recordingUrl) {
    updateData.recordingUrl = payload.recordingUrl;
  }
...
const generatePlaybackUrl = (recordingUrl: string): string => {
  ...
  const bucket = "my-streams-bucket-ap-south-1";
